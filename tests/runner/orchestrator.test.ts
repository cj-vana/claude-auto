...
describe("executeRun", () => {
	...
	it('includes issueNumber in pipeline run result when summary references an issue', async () => {
		...
		const config = { ...makeDefaultConfig(), pipeline: { enabled: true, planModel: 'opus', implementModel: 'opus', reviewModel: 'opus', maxReviewRounds: 1 } };
		mockedLoadJobConfig.mockResolvedValue(config);
		...
		expect(mockedWriteRunLog).toHaveBeenCalledWith('test-job', expect.objectContaining({ issueNumber: 42 }));
	});
});
